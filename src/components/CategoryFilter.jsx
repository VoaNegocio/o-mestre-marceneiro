      ...
      {categories.map((category) => {
        ...
        return (
          <button
            ...
            className="relative px-6 py-2.5 text-sm md:text-base font-medium transition-colors duration-300 whitespace-nowrap group rounded-full"
          >
            {/* Fundo Pílula Deslizante (apenas na categoria ativa) */}
            {isActive && (
              <motion.div
                layoutId="activeCategory"
                className="absolute inset-0 bg-brand-olive rounded-full shadow-md"
                initial={false}
                transition={{
                  type: 'spring',
                  stiffness: 500,
